...
resource "snowflake_schema" "schemas" {
  for_each = { for schema in local.flat_schemas : "${schema.database}.${schema.name}" => schema }
  ...
  with_managed_access                                       = try(each.value.with_managed_access, null)
